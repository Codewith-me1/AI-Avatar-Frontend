...
(function () {
  ...
  style.textContent = `
    ...
    .va-widget-container.is-open .va-banner { opacity: 0; pointer-events: none; transform: scale(0.85); }
    .va-widget-container.is-open .va-expanded-card { opacity: 1; pointer-events: auto; transform: scale(1) translateY(0); }

    /* media zone shrinks left when chat open */
    .va-media-zone { position: absolute; top: 0; bottom: 0; left: 0; right: 0; transition: right 0.3s cubic-bezier(0.16,1,0.3,1); }
    .va-expanded-card.chat-open .va-media-zone { right: ${CHAT_W}px; }
    ...
    .va-close-btn {
      ...
      font-size: 15px; line-height: 1; z-index: 40; transition: background 0.2s;
    }
    .va-close-btn:hover { background: rgba(0,0,0,0.7); }
    ...
    .va-signal i { width: 3px; background: #fff; border-radius: 1px; display: block; opacity: 0.9; }
    .va-signal i:nth-child(1) { height: 5px; }
    .va-signal i:nth-child(2) { height: 8px; }
    .va-signal i:nth-child(3) { height: 11px; }
    .va-timer {
      ...
      font-size: 12px; font-weight: 600; padding: 0 12px; height: 30px;
    ...
    .va-powered {
      ...
      color: rgba(255,255,255,0.8); font-size: 9px; font-weight: 600;
      letter-spacing: 0.04em; text-shadow: 0 1px 3px rgba(0,0,0,0.6);
      pointer-events: none;
    }
    .va-powered.visible { display: flex; }
    .va-expanded-card.chat-open .va-powered { right: ${CHAT_W + 12}px; }
    ...
    .va-loading-scrim { position: absolute; inset: 0; background: rgba(10,12,40,0.28); z-index: 0; }
    .va-connect-card {
      position: relative; z-index: 1;
      background: rgba(255,255,255,0.96); border-radius: 16px;
      padding: 28px 34px; display: flex; flex-direction: column; align-items: center; gap: 12px;
      box-shadow: 0 12px 40px rgba(0,0,0,0.25); max-width: 78%; text-align: center;
    }
    .va-loading-ring {
      width: 40px; height: 40px;
      border: 3px solid rgba(27,26,78,0.18); border-top-color: ${INK};
      border-radius: 50%; animation: va-spin 0.9s linear infinite;
    }
    @keyframes va-spin { to { transform: rotate(360deg); } }
    .va-connect-title { color: ${INK}; font-size: 17px; font-weight: 700; margin-top: 2px; }
    .va-connect-sub { color: #6b7280; font-size: 12px; font-weight: 500; }
    ...
    .va-expanded-card.chat-open .va-chat-panel { transform: translateX(0); }
    .va-chat-header {
    ...
    .va-chat-close:hover { color: #fff; background: rgba(255,255,255,0.1); }
    .va-chat-messages {
      flex: 1; overflow-y: auto; padding: 12px 10px;
      display: flex; flex-direction: column; gap: 8px;
    }
    .va-chat-empty { color: rgba(255,255,255,0.3); font-size: 11px; text-align: center; margin-top: 24px; }
    .va-msg { display: flex; }
    .va-msg.user  { justify-content: flex-end; }
    .va-msg.agent { justify-content: flex-start; }
    .va-msg-bubble {
    ...
    .va-chat-send:hover { background: #4338ca; }
    .va-chat-send:disabled { background: rgba(255,255,255,0.08); cursor: default; }
    .va-chat-send svg { width: 16px; height: 16px; }

    /* ── bottom controls ── */
    .va-bottom-bar {
      position: absolute; bottom: 0; left: 0; right: 0;
      display: flex; justify-content: center;
      padding: 18px 20px 22px; z-index: 30;
      opacity: 0; pointer-events: none;
    ...
    .va-tool-btn:hover { background: rgba(60,62,84,0.95); }
    .va-tool-btn svg { width: 18px; height: 18px; }
    .va-mic-btn.muted { background: rgba(239,68,68,0.85); }
    .va-tool-btn.off { background: rgba(239,68,68,0.30); border: 1px solid rgba(239,68,68,0.45); }
    .va-tool-btn.active { background: rgba(79,70,229,0.55); border: 1px solid rgba(129,140,248,0.5); }
    .va-btn-divider { width: 1px; height: 26px; background: rgba(255,255,255,0.16); margin: 0 2px; }
    .va-end-btn { background: #e5342b; width: 48px; height: 48px; }
    .va-end-btn:hover { background: #c81e15; }
    .va-end-btn svg { width: 20px; height: 20px; fill: white; }
  ...
  container.innerHTML = `
    ...
    <div class="va-expanded-card" id="vaExpandedCard">
      <div class="va-media-zone" id="vaMediaZone">
        <div class="va-status-pill" id="vaStatusPill">
          <span class="va-signal"><i></i><i></i><i></i></span>
          <span class="va-timer"><span class="va-timer-dot"></span><span id="vaTimer">0:00</span></span>
        </div>
        ...
        <div class="va-bottom-bar" id="vaBottomBar">
          <div class="va-controls-dock">
            ...
            <button class="va-circle-btn va-tool-btn va-chat-btn" id="vaChatBtn" title="Chat">
              <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
              </svg>
            </button>
      ...
      <!-- Chat panel rendered by the PARENT widget: always available -->
      <div class="va-chat-panel" id="vaChatPanel">
        <div class="va-chat-header">
          <span>Chat</span>
          <button class="va-chat-close" id="vaChatClose" title="Close chat">✕</button>
        </div>
        <div class="va-chat-messages" id="vaChatMessages">
          <div class="va-chat-empty" id="vaChatEmpty">The conversation will appear here.</div>
        </div>
        <div class="va-chat-inputrow">
          <input class="va-chat-input" id="vaChatInput" type="text" placeholder="Type a message…" autocomplete="off" />
          <button class="va-chat-send" id="vaChatSend" title="Send">
            <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  const widgetContainer = document.getElementById("vaWidgetContainer");
  const expandedCard    = document.getElementById("vaExpandedCard");
  const previewBtn      = document.getElementById("vaPreviewBtn");
  const closeBtn        = document.getElementById("vaCloseBtn");
  const endBtn          = document.getElementById("vaEndBtn");
  const micBtn          = document.getElementById("vaMicBtn");
  ...
  const timerEl         = document.getElementById("vaTimer");
  const spkBtn          = document.getElementById("vaSpkBtn");
  const chatBtn         = document.getElementById("vaChatBtn");
  const chatClose       = document.getElementById("vaChatClose");
  const chatMessages    = document.getElementById("vaChatMessages");
  ...
  // ── Chat message store: id → element (supports streaming segment updates) ──
  const msgEls = new Map();

  function upsertChatMessage(id, from, text, final) {
    if (!text || !String(text).trim()) return;
    chatEmpty.style.display = "none";
    let wrap = msgEls.get(id);
    if (!wrap) {
      wrap = document.createElement("div");
      wrap.className = `va-msg ${from === "user" ? "user" : "agent"}`;
    ...
    wrap.classList.toggle("interim", !final);
    wrap.querySelector(".va-msg-bubble").textContent = text;
    chatMessages.scrollTop = chatMessages.scrollHeight;
  }

  function clearChat() {
    msgEls.clear();
    chatMessages.querySelectorAll(".va-msg").forEach((el) => el.remove());
  ...
  function setChatOpen(open) {
    ...
    if (open) setTimeout(() => chatInput.focus(), 320);
  }
  ...
  function sendChatMessage() {
    ...
    upsertChatMessage(`local-${Date.now()}`, "user", text, true);
    // Forward into the LiveKit room via the iframe
    try {
      iframe.contentWindow.postMessage({ type: "VOICE_AGENT_SEND_TEXT", text }, "*");
    } catch (_) {}
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    switch (event.data.type) {
      ...
      // Transcript relayed from the iframe (agent + user speech + typed replies)
      // shape: { type, id, from: 'user'|'agent', text, final }
      case "VOICE_AGENT_TRANSCRIPT": {
        const { id, from, text, final } = event.data;
        upsertChatMessage(
