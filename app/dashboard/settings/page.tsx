...
function GoogleSection() {
  ...
  useEffect(() => {
    load();
    // Surface the callback's success return.
    if (typeof window !== "undefined") {
      const q = new URLSearchParams(window.location.search);
      if (q.get("google") === "connected") {
        setFlashMsg("Google Calendar connected.");
  ...
  const connect = async () => {
    ...
    try {
      ...
      setBusy(false);
      setFlashMsg("Couldn't start Google connection — try again.");
      setTimeout(() => setFlashMsg(null), 5000);
    }
  };

  const disconnect = async () => {
    if (!confirm("Disconnect Google? New bookings will use the agent's static Meet link.")) return;
    setBusy(true);
    try {
  ...
  return (
    <Section
      ...
      desc="Connect Google so each booking gets its own Calendar event and Meet link."
    >
      {flash && (
        <div className="flex items-center gap-2! text-[13px] text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-xl px-3.5! py-2.5! mb-4!">
          <Check size={15} /> {flash}
        </div>
      )}
      {loading ? (
        ...
        <div className="space-y-4!">
          <div className="flex flex-wrap items-center justify-between gap-3! p-4! rounded-xl border border-[var(--line)] bg-[var(--violet-050)]">
            <div className="flex items-center gap-3!">
              <span className="w-10! h-10! rounded-xl grid place-items-center text-white" style={{ background: "var(--grad)" }}>
                <Video size={18} />
              </span>
              <div>
                <p className="text-[14px] font-medium text-[var(--ink)]">{g.google_email}</p>
                <p className="text-[12px] text-[var(--muted)]">
                  Connected{g.connected_at ? ` ${relTime(g.connected_at)}` : ""}
                </p>
              </div>
            </div>
            <button
              onClick={disconnect}
              disabled={busy}
              className="text-[13px] font-semibold text-rose-600 border border-rose-100 hover:bg-rose-50 px-4! py-2! rounded-lg transition-colors disabled:opacity-60"
            >
              Disconnect
            </button>
          </div>
          <div
            className={`flex items-start gap-2! text-[13px] rounded-xl px-3.5! py-3! border ${
              g.calendar_ready
                ? "text-emerald-700 bg-emerald-50 border-emerald-100"
                : "text-amber-700 bg-amber-50 border-amber-100"
            }`}
          >
            {g.calendar_ready ? <CheckCircle2 size={15} className="shrink-0 mt-0.5!" /> : <AlertCircle size={15} className="shrink-0 mt-0.5!" />}
            <span>
              {g.calendar_ready
                ? "Calendar is ready — each booking gets its own Google Meet link."
                : "Calendar not ready yet — bookings fall back to the agent's static Meet room until Google finishes authorising."}
            </span>
          </div>
        </div>
      ) : (
        <div>
          <p className="text-[13px] text-[var(--slate)] mb-4!">
            Not connected. Without Google, bookings use the agent&apos;s static Meet
            link (set per-agent in CRM settings).
          </p>
          <button
            onClick={connect}
            disabled={busy}
            className="inline-flex items-center gap-2.5! bg-white border border-[var(--line)] text-[var(--ink)] text-[13px] font-semibold px-4! py-2.5! rounded-xl hover:bg-[var(--line-soft)] transition-colors disabled:opacity-70"
          >
            {busy ? (
          ...
          <p className="flex items-center gap-1.5! text-[12px] text-[var(--muted)] mt-3!">
            <Link2 size={12} /> Grants calendar access to create events with Meet links.
          </p>
        </div>
      )}
    </Section>
